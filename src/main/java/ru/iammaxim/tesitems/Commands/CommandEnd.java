...
public class CommandEnd extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        float attr = cap.getAttribute("endurance");
        int value = (int) Math.round((attr + Math.random() * (200 - attr)) / 10);
        String message = TextFormatting.LIGHT_PURPLE + "*" + player.getName() + " " + String.join(" ", args) + ": " + TextFormatting.YELLOW + value + " (endurance)" + TextFormatting.LIGHT_PURPLE + "*";
